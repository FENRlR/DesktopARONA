...
class ArgsBase():
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--train_file',
                            type=str,
                            default='./KoBARTchatbot/Chatbot_data/train.csv',
                            help='train file')
        parser.add_argument('--test_file',
                            type=str,
                            default='./KoBARTchatbot/Chatbot_data/test.csv',
                            help='test file')
...
class ChatDataset(Dataset):
    def __init__(self, filepath, tok_vocab, max_seq_len=128) -> None:
        ...
        self.tokenizer = PreTrainedTokenizerFast(
            ...
            bos_token=self.bos_token, eos_token=self.eos_token, unk_token='<unk>', pad_token='<pad>',
            mask_token='<mask>')
...
class Base(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        num_workers = gpunumber * self.hparams.num_nodes
        data_len = len(self.train_dataloader().dataset)
        logging.info(f'number of workers {num_workers}, data length {data_len}')
        self.hparams.max_epochs = 3
